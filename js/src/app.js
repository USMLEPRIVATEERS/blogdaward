...
function _handle401() {
    ...
    if (window.location.pathname === '/' || window.location.pathname === '/index' || window.location.pathname === '/register') return;
...
// Routes all requests through /api/ serverless functions
class _QB {
    constructor(table) {
        this._t = table;
        this._a = 'select';
        ...
        this._c = null;
        this._uo = null;
        this._selectAfterWrite = undefined;
    }
    select(cols, opts) {
        // If called after insert/update/upsert, it means "return the written row"
        if (this._a === 'insert' || this._a === 'update' || this._a === 'upsert') {
            this._selectAfterWrite = cols || '*';
        } else {
            this._a = 'select';
        }
        if (cols !== undefined) this._s = cols;
        if (opts && opts.count) this._c = opts.count;
        return this;
    }
    insert(d) { this._a = 'insert'; this._d = d; return this; }
    update(d) { this._a = 'update'; this._d = d; return this; }
    delete() { this._a = 'delete'; return this; }
    upsert(d, opts) { this._a = 'upsert'; this._d = d; this._uo = opts; return this; }
    eq(c, v) { this._f.push({type:'eq',col:c,val:v}); return this; }
    neq(c, v) { this._f.push({type:'neq',col:c,val:v}); return this; }
    gt(c, v) { this._f.push({type:'gt',col:c,val:v}); return this; }
    gte(c, v) { this._f.push({type:'gte',col:c,val:v}); return this; }
    lt(c, v) { this._f.push({type:'lt',col:c,val:v}); return this; }
    lte(c, v) { this._f.push({type:'lte',col:c,val:v}); return this; }
    in(c, v) { this._f.push({type:'in',col:c,val:v}); return this; }
    is(c, v) { this._f.push({type:'is',col:c,val:v}); return this; }
    like(c, v) { this._f.push({type:'like',col:c,val:v}); return this; }
    ilike(c, v) { this._f.push({type:'ilike',col:c,val:v}); return this; }
    contains(c, v) { this._f.push({type:'contains',col:c,val:v}); return this; }
    not(c, op, v) { this._f.push({type:'not',col:c,op:op,val:v}); return this; }
    or(expr) { this._f.push({type:'or',expr:expr}); return this; }
    match(v) { this._f.push({type:'match',val:v}); return this; }
    filter(c, op, v) { this._f.push({type:'filter',col:c,op:op,val:v}); return this; }
    order(c, opts) {
        if (!this._o) this._o = [];
        if (!Array.isArray(this._o)) this._o = [this._o];
        this._o.push({col:c, ascending: opts ? opts.ascending !== false : true});
        return this;
    }
    limit(n) { this._l = n; return this; }
    range(from, to) { this._r = {from:from, to:to}; return this; }
    single() { this._si = true; return this; }
    maybeSingle() { this._ms = true; return this; }
    ...
    async _exec() {
        try {
            const body = {
                ...
                action: this._a,
                select: this._s,
                filters: this._f.length ? this._f : undefined,
                data: this._d,
                order: this._o,
                limit: this._l,
                range: this._r,
                single: this._si || undefined,
                maybeSingle: this._ms || undefined,
                count: this._c,
                upsertOpts: this._uo
            };
            ...
            if (r.status === 401) { _handle401(); return { data: null, error: { message: 'Session expired' } }; }
            return await r.json();
        } catch(e) {
...
function _getAuthToken() {
    try {
        const sk = Object.keys(localStorage).find(k => k.startsWith('sb-') && k.endsWith('-auth-token'));
        if (sk) {
...
function _getWardToken() {
    return localStorage.getItem('_ward_tk') || null;
}

function _buildHeaders() {
    const h = {'Content-Type': 'application/json'};
    const at = _getAuthToken();
    if (at) h['x-supabase-auth'] = at;
    const wt = _getWardToken();
    if (wt) h['x-ward-token'] = wt;
    return h;
}

// Proxy client that replaces direct Supabase client
const supabaseClient = {
    from: function(table) { return new _QB(table); },
    rpc: async function(fn, params) {
        try {
            const r = await fetch('/api/data/rpc', {
                method: 'POST', headers: _buildHeaders(),
                body: JSON.stringify({function_name: fn, params: params || {}})
            });
            if (r.status === 401) { _handle401(); return { data: null, error: { message: 'Session expired' } }; }
            return await r.json();
        } catch(e) {
    ...
    auth: {
        signInWithPassword: async function(creds) {
            try {
                const r = await fetch('/api/auth/login', {
                    ...
                    body: JSON.stringify({...creds, action: 'signInWithPassword'})
                });
                const result = await r.json();
                if (result.data && result.data.session) {
                    const sk = 'sb-ward-auth-token';
                    localStorage.setItem(sk, JSON.stringify(result.data.session));
                }
                return result;
            } catch(e) {
                return { data: null, error: { message: e.message } };
        ...
        updateUser: async function(userData) {
            try {
                const r = await fetch('/api/auth/login', {
                    method: 'POST', headers: _buildHeaders(),
                    body: JSON.stringify({action: 'updateUser', userData: userData})
                });
                return await r.json();
            } catch(e) {
                return { data: null, error: { message: e.message } };
        ...
        admin: {
            createUser: async function(userData) {
                try {
                    const r = await fetch('/api/auth/admin', {
                        method: 'POST', headers: _buildHeaders(),
                        body: JSON.stringify({action: 'createUser', userData: userData})
                    });
                    return await r.json();
                } catch(e) {
                    return { data: null, error: { message: e.message } };
            ...
            updateUserById: async function(userId, userData) {
                try {
                    const r = await fetch('/api/auth/admin', {
                        method: 'POST', headers: _buildHeaders(),
                        body: JSON.stringify({action: 'updateUserById', userId: userId, userData: userData})
                    });
                    return await r.json();
                } catch(e) {
                    return { data: null, error: { message: e.message } };
...
// Make it globally available (replaces the real Supabase client)
...
async function login(cpf, password) {
    ...
    try {
        ...
        if (!data.first_login_completed) {
            ...
            const stepUrls = {
                0: '/questionnaire/step1-basic-data',
                1: '/questionnaire/step2-usmle-data',
                2: '/questionnaire/step3-uworld-prep',
                3: '/questionnaire/step4-uworld-progress',
                4: '/questionnaire/step5-english',
                5: '/questionnaire/step6-anki',
                6: '/questionnaire/step7-research-1',
                7: '/questionnaire/step8-research-2',
                8: '/questionnaire/step9-research-3',
                9: '/questionnaire/step10-observerships',
                10: '/questionnaire/step11-background',
                11: '/questionnaire/step11-background'
            };
            ...
            switch (data.role) {
                case 'mentor_marcos':
                    window.location.href = '/mentor-dashboard-marcos';
                    break;
                case 'mentor_iria':
                    window.location.href = '/mentor-dashboard-iria';
                    break;
...
function updateNavbarForRole() {
    ...
    switch (user.role) {
        case 'mentor_marcos':
            dashboardLink.href = '/mentor-dashboard-marcos';
            break;
        case 'mentor_iria':
            dashboardLink.href = '/mentor-dashboard-iria';
            break;
        case 'mentor_guilherme':
    ...
    if (user.role && user.role.startsWith('mentor_')) {
        ...
        if (navMenu) {
            ...
            if (!navMenu.querySelector('a[href="/admin-members"]')) {
                // Add admin link for Marcos
                if (user.role === 'mentor_marcos') {
                    const adminLink = document.createElement('a');
                    adminLink.href = '/admin-members';
