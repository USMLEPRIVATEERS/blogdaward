...
CREATE POLICY "Users can read own responses" ON flash_question_responses
    FOR SELECT USING (auth.uid() = user_id);

-- Usuários podem inserir suas próprias respostas
CREATE POLICY "Users can insert own responses" ON flash_question_responses
    FOR INSERT WITH CHECK (auth.uid() = user_id);

-- Políticas para flash_tests
-- Usuários podem ver seus próprios testes
CREATE POLICY "Users can read own tests" ON flash_tests
    FOR SELECT USING (auth.uid() = user_id);

-- Usuários podem criar seus próprios testes
CREATE POLICY "Users can insert own tests" ON flash_tests
    FOR INSERT WITH CHECK (auth.uid() = user_id);

-- Usuários podem atualizar seus próprios testes
CREATE POLICY "Users can update own tests" ON flash_tests
...
CREATE POLICY "Authenticated users can insert comments" ON flash_question_comments
    FOR INSERT WITH CHECK (auth.uid() = user_id);

-- Usuários podem atualizar/deletar seus próprios comentários
CREATE POLICY "Users can update own comments" ON flash_question_comments
    FOR UPDATE USING (auth.uid() = user_id);

CREATE POLICY "Users can delete own comments" ON flash_question_comments
